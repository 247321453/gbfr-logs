...
impl Parser {
        ...
        if self.status == ParserStatus::InProgress {
            ...
            if self.has_damage() {
                match self.save_encounter_to_db() {
                    ...
                    Err(e) => {
                        if let Some(app) = &self.app {
                            let _ = app.emit_str("encounter-saved-error", e.to_string());
                        }
                    }
                }
            }
    ...
    pub fn on_player_load_event(&mut self, event: PlayerLoadEvent) {
        ...
        // Insert into encounter player data array by actor_index. party_index is
        // unavailable in game 2.0 compatibility mode (sigil_offset unresolved), so
        // slot 0 is no longer reserved for the local player specifically - all four
        // slots are filled/updated uniformly, sorted by actor_index.
        for i in 0..=3 {
